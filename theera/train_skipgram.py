...
data = pandas.read_parquet('python3 train_skipgram.py/home/dataset/Flores7Lang.parquet')
...
skipgram = model.SkipGram(len(ds.tokenizer.vocab), 50)
...
for epoch in range(5):
  ...
  for context, target in tqdm.tqdm(dl, desc=f"Epoch {epoch+1}/10", unit="batch"):
    optimizer.zero_grad()
    log_probs = skipgram(context)
    loss = loss_function(log_probs, target)
    loss.backward()
    optimizer.step()
    total_loss += loss.item()
  print(f"Epoch {epoch+1}/10, Loss: {total_loss}")
  torch.save(skipgram.state_dict(), f"./skipgram_epoch_{epoch+1}.pt")